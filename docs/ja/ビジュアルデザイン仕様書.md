...
タスバルでは **`flutter_liquid_glass_plus`** パッケージを使用してLiquid Glassデザインを実装する。

| 項目 | 内容 |
|------|------|
| パッケージ名 | `flutter_liquid_glass_plus` |
| pub.dev | https://pub.dev/packages/flutter_liquid_glass_plus |
| 用途 | Apple Liquid GlassスタイルのUIコンポーネント提供 |

このパッケージにより、以下のデザイン要素を統一的に実装する：

- **半透明ガラス効果** - カード、シート、ダイアログの背景
- **ブラー効果** - 背景のぼかし処理
- **光の反射・屈折** - ハイライト、グラデーション
- **滑らかなアニメーション** - 状態遷移、タップフィードバック
