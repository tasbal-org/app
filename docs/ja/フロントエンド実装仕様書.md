...
### 0.1 Liquid Glass UI の実装

タスバルのUIは **`flutter_liquid_glass_plus`** パッケージを主要UIフレームワークとして使用する。

#### パッケージ情報

| 項目 | 内容 |
|------|------|
| パッケージ名 | `flutter_liquid_glass_plus` |
| 目的 | Apple Liquid Glassスタイルのガラス質UIを実現 |
| 適用範囲 | 全てのUI要素（カード、ボタン、シート、ダイアログ等） |

#### 使用コンポーネント

| コンポーネント | 用途 |
|--------------|------|
| LiquidGlassCard | タスクカード、セクションコンテナ |
| LiquidGlassButton | アクションボタン、FAB |
| LiquidGlassBottomSheet | タスク作成・編集シート |
| LiquidGlassDialog | 確認ダイアログ |
| LiquidGlassTextField | テキスト入力フィールド |
| LiquidGlassChip | フィルタチップ、タグ表示 |
| LiquidGlassListTile | リスト項目 |
| LiquidGlassTabBar | タブナビゲーション |

#### 実装ガイドライン

```dart
// 基本的なLiquid Glassスタイルの適用
...
- 標準のFlutter MaterialウィジェットよりもLiquid Glassウィジェットを優先して使用する
- カスタムウィジェットを作成する場合も、Liquid Glassの視覚スタイルに準拠する
- パフォーマンスに配慮し、ブラー効果は必要な箇所に限定する
